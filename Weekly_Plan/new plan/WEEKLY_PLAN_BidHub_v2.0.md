# BidHub — Lộ trình 10 tuần (Phiên bản 2.0 · Đạt điểm A+)
...
> **v2.0 changelog:** Refactor `ItemFactory` (Static Factory) → `ItemCreator` hierarchy (Factory Method Pattern chuẩn GoF). Xem chi tiết tại Tuần 2 · Quốc Minh.
...
**Creator hierarchy (thay thế `ItemFactory.java`):**
...
├── Electronics.java        ← Concrete Product (final)
├── Art.java                ← Concrete Product (final)
├── Vehicle.java            ← Concrete Product (final)
├── ItemCreator.java        ← Abstract Creator — factory method createItem()
├── ElectronicsCreator.java ← Concrete Creator (final)
├── ArtCreator.java         ← Concrete Creator (final)
└── VehicleCreator.java     ← Concrete Creator (final)
```

**Lợi ích Open/Closed Principle:** Khi thêm `ItemType.JEWELRY`, chỉ cần tạo `Jewelry extends Item` và `JewelryCreator extends ItemCreator`. Không cần sửa `ElectronicsCreator`, `ArtCreator`, `VehicleCreator`, hay bất kỳ code client nào đang dùng `ItemCreator.forType()`.

**✅ Đầu ra:**
- `new ElectronicsCreator().createItem(...)` → `instanceof Electronics`
- `ItemCreator.forType(ItemType.ART)` → `instanceof ArtCreator`
- `List<Item>` gọi `getCategoryDetails()` → 3 output khác nhau (polymorphism qua abstract method)
- `item instanceof Entity` = `true` (kế thừa Electronics→Item→Entity)
- **Không có** `ItemFactory.java` utility class (đã thay bằng Creator hierarchy)
- `startingPrice = -100` → `IllegalArgumentException`
- `ItemCreator.forType(null)` → `NullPointerException`
...
`handleCreateItem()` dùng `ItemCreator.forType(itemType).createItem(name, description, startingPrice, sellerId, extras)` để tạo đúng loại Item từ Creator hierarchy (không dùng `ItemFactory.create()`).
...
- `CREATE_ITEM` với type="ELECTRONICS" → `ItemCreator.forType(ELECTRONICS).createItem(...)` → `Electronics` instance
...
- Tuần 2: 14+ cases (Entity, UserHierarchy, ItemCreatorTest, Exception hierarchy)
...
Nội dung bắt buộc trong `docs/DESIGN_PATTERNS.md`:

**Singleton Pattern:**
Các class áp dụng: `DbConnectionProvider`, `AuctionManager`, `SessionManager`, `NotificationBroker`, `ViewRouter`. Lợi ích: đảm bảo chỉ có 1 instance kết nối DB / 1 broker quản lý subscriber toàn hệ thống.

**Factory Method Pattern:**

Classes áp dụng:
- Creator: `ItemCreator` (abstract)
- ConcreteCreator: `ElectronicsCreator`, `ArtCreator`, `VehicleCreator`
- Product: `Item` (abstract)
- ConcreteProduct: `Electronics`, `Art`, `Vehicle`

Factory Method: `ItemCreator.createItem(String, String, double, String, Map<String, Object>)`

Lợi ích trong BidHub: Khi thêm `ItemType.JEWELRY`, chỉ cần tạo `Jewelry extends Item` và `JewelryCreator extends ItemCreator`. Không cần sửa `ElectronicsCreator`, `ArtCreator`, `VehicleCreator`, hay bất kỳ code client nào đang dùng `ItemCreator.forType()`. Đây là minh chứng cho Open/Closed Principle.

Sequence diagram Factory Method (Mermaid):

```mermaid
sequenceDiagram
    participant Client
    participant ItemCreator
    participant ElectronicsCreator
    participant Electronics

    Client->>ItemCreator: forType(ELECTRONICS)
    ItemCreator-->>Client: ElectronicsCreator instance

    Client->>ElectronicsCreator: createItem("MacBook", ..., extras)
    ElectronicsCreator->>Electronics: new Electronics(name, ..., brand, warrantyMonths)
    Electronics-->>ElectronicsCreator: instance
    ElectronicsCreator-->>Client: Item (runtime type: Electronics)
```

**Observer Pattern:**
Các class áp dụng: `NotificationBroker` (subject/broker), `Session` (observer), `BidUpdateEvent` / `AuctionClosedEvent` / `AuctionExtendedEvent` (event). Lợi ích: push realtime đến tất cả client đang xem phiên mà không cần polling.
...
- `DESIGN_PATTERNS.md` có đủ 3 pattern, 4 thành phần GoF của Factory Method, sequence diagram
...
- [x] 1.0đ Design Patterns:
    ...
    - Observer: NotificationBroker + BidUpdateEvent
...
| Design Patterns (Singleton × 5, **Factory Method chuẩn GoF**, Observer) | 1.0 | Tuần 3, 4, 7 | Đăng + Quốc Minh |
